...
function cleanup_linux {
	...
	dirs_to_clean=""
	dirs_to_clean=$(echo /var/run/dpdk/spdk /var/run/dpdk/spdk_pid /tmp/dpdk/spdk /tmp/dpdk/spdk_pid)
	if [[ -d $XDG_RUNTIME_DIR && $XDG_RUNTIME_DIR != *" "* ]]; then
		dirs_to_clean+="$(readlink -e assert_not_empty $XDG_RUNTIME_DIR/dpdk/spdk || true) "
		dirs_to_clean+="$(readlink -e assert_not_empty $XDG_RUNTIME_DIR/dpdk/spdk_pid || true) "
	fi
